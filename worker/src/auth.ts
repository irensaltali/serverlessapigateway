...
async function jwtAuth(request: Request): Promise<JWTPayload> {
    ...
    try {
        ...
        if (error instanceof errors.JOSEAlgNotAllowed) {
            throw new AuthError('Algorithm not allowed: ' + error.message, error.code, 401);
        } else if (error instanceof errors.JWEDecryptionFailed) {
            throw new AuthError('Decryption failed: ' + error.message, error.code, 401);
        } else if (error instanceof errors.JWEInvalid) {
            throw new AuthError('Invalid JWE: ' + error.message, error.code, 401);
            ...
            throw new AuthError('JWT claim validation failed: ' + error.message, error.code, 401);
        } else if (error instanceof errors.JWTInvalid) {
            throw new AuthError('Invalid JWT: ' + error.message, error.code, 401);
            ...
            throw new AuthError('Invalid JWKS: ' + error.message, error.code, 401);
            ...
            throw new AuthError('Invalid JWS: ' + error.message, error.code, 401);
        } else if (error instanceof errors.JWSSignatureVerificationFailed) {
            throw new AuthError('Signature verification failed: ' + error.message, error.code, 401);
        } else if (error instanceof Error) {
            throw new AuthError('JWT verification failed: ' + error.message, "AUTH_ERROR", 401);
        }
